...
# Complex matrix types in PARDISO
_COMPLEX_MTYPES = {3, 4, -4, 6, 13}
# Symmetric matrix types in PARDISO
_SYMMETRIC_MTYPES = {-2, 2, -4, 4, 6}

def _find_mkl_rt():
    """Find and load mkl_rt shared library.
    
    Returns the ctypes handle to mkl_rt, or None if not found.
    """
    libmkl = None
...
# Module-level cached handle (avoids re-searching on every solver instantiation)
_MKL_RT_HANDLE = _find_mkl_rt()
...
class PyPardisoSolver:
    ...
    def __init__(self, mtype=11, phase=13, size_limit_storage=5e7):
        ...
        # Reuse module-level pre-loaded mkl_rt handle (loaded with RTLD_GLOBAL
        # to prevent conflicts with other MKL-linked extensions like vbcsr).
        if _MKL_RT_HANDLE is not None:
            self.libmkl = _MKL_RT_HANDLE
        else:
            raise ImportError(
    ...
    def _is_complex(self):
        """Return True if the current mtype is a complex matrix type."""
        return self.mtype in _COMPLEX_MTYPES

    @property
    def _dtype(self):
        """Return the expected numpy dtype for the current mtype."""
        return np.complex128 if self._is_complex else np.float64
    ...
    def factorize(self, A):
        ...
        self.set_phase(12)
        b = np.zeros((A.shape[0], 1), dtype=self._dtype)
    ...
    def _check_A(self, A):
        ...
        if A.dtype != expected_dtype:
            raise TypeError(
                'Matrix A has dtype {}, but mtype={} requires dtype {}'.format(
                    A.dtype, self.mtype, expected_dtype))
    ...
    def _check_b(self, A, b):
        ...
        if b.dtype != expected_dtype:
            if self._is_complex:
                # For complex types, try to cast compatible dtypes
                if np.issubdtype(b.dtype, np.complexfloating) or np.issubdtype(b.dtype, np.floating):
                    warnings.warn(
                        "Array b's data type was converted from {} to {}".format(b.dtype, expected_dtype),
                        PyPardisoWarning)
                    b = b.astype(expected_dtype)
                else:
                    raise TypeError('Dtype {} for array b is not supported'.format(b.dtype))
            else:
    ...
    def _call_pardiso(self, A, b):
        ...
        self._mkl_pardiso(self.pt.ctypes.data_as(ctypes.POINTER(self._pt_type[0])),  # pt
                          ...
                          A.data.ctypes.data_as(ctypes.c_void_p),  # A -> non-zero entries in matrix
                          ...
                          b.ctypes.data_as(ctypes.c_void_p),  # b -> right-hand side vector/matrix
                          x.ctypes.data_as(ctypes.c_void_p),  # x -> output
