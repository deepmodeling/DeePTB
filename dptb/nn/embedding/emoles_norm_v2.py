...
from dptb.nn.norm import MySeperableLayerNorm as SeperableLayerNorm
...
def apply_optional_conditioned_norm(
    norm_module: nn.Module,
    x: torch.Tensor,
    conditioning: Optional[torch.Tensor] = None,
    use_condition: bool = False,
):
    if getattr(norm_module, "supports_conditioning", False):
        return norm_module(
            x,
...
class InitLayer(torch.nn.Module):
    def __init__(
            ...
            ln_flag: bool = True,
            norm_build_node_condition_branch: bool = False,
            norm_use_node_onehot: bool = False,
    ):
        ...
        self.device = device
        self.dtype = dtype
        self.norm_build_node_condition_branch = norm_build_node_condition_branch
        self.norm_use_node_onehot = norm_use_node_onehot
        ...
        if ln_flag:
            cond_dim = self.num_types if self.norm_build_node_condition_branch else 0
    ...
    def forward(
        ...
        norm_condition = None
        if self.norm_use_node_onehot and node_onehot is not None:
            if node_features.shape[0] < node_onehot.shape[0]:
                norm_condition = node_onehot[:node_features.shape[0]]
            else:
                norm_condition = node_onehot

        node_features = apply_optional_conditioned_norm(
            self.sln_n,
            node_features,
            conditioning=norm_condition,
            use_condition=(self.norm_use_node_onehot and norm_condition is not None),
        )
...
class EAMP(torch.nn.Module):
    def __init__(
            ...
            norm_build_node_condition_branch: bool = False,
            norm_use_node_onehot: bool = False,
            norm_build_edge_condition_branch: bool = False,
            norm_use_edge_onehot: bool = False,
    ):
        ...
        self.edge_condition_dim = edge_condition_dim
        self.dtype = dtype
        self.device = device
        ...
        self.norm_build_node_condition_branch = norm_build_node_condition_branch
        self.norm_use_node_onehot = norm_use_node_onehot
        self.norm_build_edge_condition_branch = norm_build_edge_condition_branch
        self.norm_use_edge_onehot = norm_use_edge_onehot
        ...
        node_cond_dim = self.node_one_hot_dim if self.norm_build_node_condition_branch else 0
        edge_cond_dim = self.edge_condition_dim if self.norm_build_edge_condition_branch else 0
        ...
        if self.self_mix_flag:
            ...
            if "node" in self.self_mix_type or "all" in self.self_mix_type:
                node_mixer_cond_dim = self.node_one_hot_dim if self.norm_build_node_condition_branch else 0
                self.node_mixer = self._build_mixer_module(cond_dim=node_mixer_cond_dim)

            if "edge" in self.self_mix_type or "all" in self.self_mix_type:
                edge_mixer_cond_dim = self.edge_condition_dim if self.norm_build_edge_condition_branch else 0
                self.edge_mixer = self._build_mixer_module(cond_dim=edge_mixer_cond_dim)
    ...
    def _build_mixer_module(self, cond_dim: int = 0):
    ...
    def _apply_self_mix(
        ...
        normed_features = apply_optional_conditioned_norm(
            mixer["norm"],
            current_features,
            conditioning=conditioning,
            use_condition=use_condition,
    ...
    def forward(
        ...
        node_norm_condition = node_onehot if self.norm_use_node_onehot else None
        edge_norm_condition = edge_onehot if self.norm_use_edge_onehot else None

        norm_node_features = apply_optional_conditioned_norm(
            self.sln_n,
            node_features,
            conditioning=node_norm_condition,
            use_condition=(self.norm_use_node_onehot and node_norm_condition is not None),
        )

        norm_edge_features = apply_optional_conditioned_norm(
            self.sln_e,
            edge_features,
            conditioning=edge_norm_condition,
            use_condition=(self.norm_use_edge_onehot and edge_norm_condition is not None),
        )
        ...
        if self.optimized_in_frame:
            ...
            if self.self_mix_flag and self.edge_mixer is not None:
                edge_mix_out = self._apply_self_mix(
                    self.edge_mixer,
                    edge_messages,
                    conditioning=edge_norm_condition,
                    use_condition=(self.norm_use_edge_onehot and edge_norm_condition is not None),
        ...
        if self.self_mix_flag and self.node_mixer is not None:
            node_mix_out = self._apply_self_mix(
                self.node_mixer,
                node_features,
                conditioning=node_norm_condition,
                use_condition=(self.norm_use_node_onehot and node_norm_condition is not None),
...
class EMolES(torch.nn.Module):
    def __init__(
        ...
        if norm_use_node_onehot and (not norm_build_node_condition_branch):
            raise ValueError("norm_use_node_onehot=True requires norm_build_node_condition_branch=True.")
        if norm_use_edge_onehot and (not norm_build_edge_condition_branch):
            raise ValueError("norm_use_edge_onehot=True requires norm_build_edge_condition_branch=True.")
        ...
        self.init_layer = InitLayer(
            ...
            norm_build_node_condition_branch=norm_build_node_condition_branch,
            norm_use_node_onehot=norm_use_node_onehot,
        )
        ...
        for i in range(n_layers):
            ...
            self.layers.append(
                EAMP(
                    ...
                    edge_condition_dim=edge_one_hot_dim,
                    norm_eps=norm_eps,
                    ...
                    norm_build_node_condition_branch=norm_build_node_condition_branch,
                    norm_use_node_onehot=norm_use_node_onehot,
                    norm_build_edge_condition_branch=norm_build_edge_condition_branch,
                    norm_use_edge_onehot=norm_use_edge_onehot,
                )
            )
...
@Embedding.register("emoles_openequi_norm_v2")
class EMolESOpenequiNormV2(EMolES):
    def __init__(self, **kwargs):
        ...
        base_layer_kwargs = {
            ...
            "norm_bottleneck_ratio": kwargs.get('norm_bottleneck_ratio', 0.1),
            "norm_build_node_condition_branch": kwargs.get('norm_build_node_condition_branch', False),
            "norm_use_node_onehot": kwargs.get('norm_use_node_onehot', False),
            "norm_build_edge_condition_branch": kwargs.get('norm_build_edge_condition_branch', False),
            "norm_use_edge_onehot": kwargs.get('norm_use_edge_onehot', False),
            "edge_condition_dim": edge_one_hot_dim,
        }
