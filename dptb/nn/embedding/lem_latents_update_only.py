...
from .edge_angle_tools.edge_angle_utils import compute_cos_angle
...
class Lem(torch.nn.Module):
    def __init__(
        ...
        self.edge_one_hot = OneHotEdgeEmbedding(num_types=self.n_atom, idp=self.idp, universal=universal, d_emb=edge_one_hot_dim)
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        latents, node_features, edge_features, asym_latents, asym_node_features, asym_edge_features, cutoff_coeffs, active_edges, lat_vec_len_feature, cos_angles_feature = self.init_layer(edge_index, atom_type, bond_type, edge_sh, edge_length, edge_one_hot, data)

        # print('Use asym_latents')
        latents = asym_latents
...
class InitLayer(torch.nn.Module):
    def __init__(
        ...
        if self.use_angle:
            ...
            self.rbf_angle = torch.nn.Sequential(
                RBFExpansion(
                    vmin=-1.0,
                    vmax=1.0,
                    bins=latent_dim,
            ...
            self.update_latents = ComformerConv_edge(
    ...
    def forward(self, edge_index, atom_type, bond_type, edge_sh, edge_length, edge_one_hot, data):
        ...
        if not self.use_angle:
            raise RuntimeError('USE angle!')

        cos_angles, lattice_vectors = compute_cos_angle(data)
        lattice_vectors_len = -0.75 / torch.norm(lattice_vectors, dim=-1)
        lat_vec_len_feature = self.rbf(lattice_vectors_len.reshape(-1)).reshape(cos_angles.shape[0], 3, -1)
        cos_angles_feature = self.rbf_angle(cos_angles.reshape(-1)).reshape(cos_angles.shape[0], 3, -1)
        asym_latents = self.update_latents(
            new_latents, lat_vec_len_feature[prev_mask], cos_angles_feature[prev_mask]
        )
        ...
        # Apply cutoff, which propagates through to everything else
        asym_latents = torch.index_copy(
            latents, 0, active_edges,
            cutoff_coeffs[active_edges].unsqueeze(-1) * asym_latents
            )

        weights_e = self.env_embed_mlp(asym_latents[prev_mask])
        # features = self.bn(features)

        asym_edge_features = self._env_weighter(
            edge_sh[prev_mask], weights_e
        )

        asym_node_features = scatter(
            asym_edge_features,
            edge_center[active_edges],
            dim=0,
        )
...
############################################################################################
        ...
        ############################################################################################
        ############################################################################################
        asym_node_features = asym_node_features * norm_const
        asym_node_features = self.sln_n(asym_node_features)
        # the radial embedding x and the sperical hidden V
        return latents, node_features, edge_features,  asym_latents, asym_node_features, asym_edge_features, cutoff_coeffs, active_edges, lat_vec_len_feature[prev_mask], cos_angles_feature[prev_mask]
...
class UpdateEdge(torch.nn.Module):
    def __init__(
        ...
        self.ln = torch.nn.LayerNorm(latent_dim, eps=norm_eps)
    ...
    def forward(self, latents, node_features, node_onehot, edge_features, edge_index, edge_vector, cutoff_coeffs, active_edges, edge_one_hot, lat_vec_len_feature, cos_angles_feature):
        edge_center = edge_index[0]
        edge_neighbor = edge_index[1]
        ...
        new_latents = self.update_latents(
            new_latents, lat_vec_len_feature, cos_angles_feature
        )
...
class Layer(torch.nn.Module):
    ...
    def forward(self, latents, node_features, edge_features, node_onehot, edge_index, edge_vector, atom_type, cutoff_coeffs, active_edges, edge_one_hot, lat_vec_len_feature, cos_angles_feature):
        
        edge_features, latents = self.edge_update(latents, node_features, node_onehot, edge_features, edge_index, edge_vector, cutoff_coeffs, active_edges, edge_one_hot, lat_vec_len_feature, cos_angles_feature)
