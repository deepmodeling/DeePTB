...
class Lem(torch.nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        if self.use_out_onehot_tp:
            ...
            out_node_features_one_hot = self.out_node_ele_tp(node_features, node_one_hot)
            data[_keys.NODE_FEATURES_KEY] = out_node_features + out_node_features_one_hot

            out_edge_features_one_hot = self.out_edge_ele_tp(edge_features, edge_one_hot)
            out_edge_features = out_edge_features + out_edge_features_one_hot
        else:
...
class UpdateNode(torch.nn.Module):
    def __init__(
        ...
        if use_layer_onehot_tp:
            self.node_onehot_tp = FullyConnectedTensorProduct(self.irreps_out, f'95x0e', self.irreps_out)
...
class UpdateEdge(torch.nn.Module):
    def __init__(
        ...
        self.use_layer_onehot_tp = use_layer_onehot_tp
        if use_layer_onehot_tp:
            self.edge_onehot_tp = FullyConnectedTensorProduct(self.irreps_out, f'{edge_one_hot_dim}x0e', self.irreps_out)
