...
class MOLERouter(nn.Module):
    """Computes routing coefficients from global features with Top-K support and Z-Loss."""
    ...
    def forward(self, global_features):
        ...
        # [新增] 计算 Z-Loss: mean(max(probs)^2)
        # 必须基于原始的 Softmax 概率计算，作为正则项抑制某些 Expert 过于强势
        probs_all = F.softmax(logits, dim=-1)
        max_prob = probs_all.max(dim=-1)[0]
        z_loss = torch.mean(max_prob ** 2)
        ...
        if self.top_k is not None and 0 < self.top_k < logits.shape[-1]:
            ...
            # 返回 coeffs 和 z_loss
            return coeffs, z_loss
        else:
            # 原始全激活逻辑
            return F.softmax(logits, dim=-1) + 0.005, z_loss
