...
class Band(ElecStruCal):
    ...
    def get_bands(self, data: Union[AtomicData, ase.Atoms, str], kpath_kwargs: dict, pbc: Union[bool, list] = None,
        ...
        if isinstance(data, str):
            ...
            raise ValueError("Unsupported data type for Band.get_bands")

        if kline_type == 'ase':
            kpath = kpath_kwargs['kpath']
            nkpoints = kpath_kwargs['nkpoints']
            klist, xlist, high_sym_kpoints, labels = ase_kpath(structase=structase, pathstr=kpath,
                                                               total_nkpoints=nkpoints)
            ...
            klist, xlist, high_sym_kpoints, labels = vasp_kpath(structase=structase,
                                                                pathstr=kpath,
                                                                high_sym_kpoints_dict=high_sym_kpoints_dict,
        ...
        override_full_h = kpath_kwargs.get("override_full_h", None)

        # NEW tags:
        override_full_h_uureal = kpath_kwargs.get("override_full_h_uureal", None)
        override_full_h_wo_uureal = kpath_kwargs.get("override_full_h_wo_uureal", None)

        eig_solver = kpath_kwargs.get("eig_solver", None)

        data, eigenvalues = self.get_eigs(
            data=data, klist=klist, pbc=pbc,
            AtomicData_options=AtomicData_options,
            override_overlap=override_overlap,
            add_h0=add_h0,
            override_full_h=override_full_h,
            override_full_h_uureal=override_full_h_uureal,         # NEW
            override_full_h_wo_uureal=override_full_h_wo_uureal,   # NEW
    ...
    def band_plot(
        ...
        if E_fermi is not None:
            if abs(self.eigenstatus["E_fermi"] - E_fermi) > 1e-3:
                log.info(
                    f"Using input Fermi energy: {E_fermi:.4f} eV (estimated: {self.eigenstatus['E_fermi']:.4f} eV)")
            else:
        ...
        if ref_band is not None:
            ...
            if ref_band.shape[0] != self.eigenstatus["eigenvalues"].shape[0]:
                log.error("Reference Eigenvalues' should have sampled from the same kpath as model's prediction.")
                raise ValueError

            min_ref = np.min(ref_band)
            min_pred = np.min(self.eigenstatus["eigenvalues"])
            ref_band = ref_band - (min_ref - min_pred)

            nintp = self.eigenstatus["xlist"].shape[0] // 25

            band_ref = ax.plot(self.eigenstatus["xlist"][::nintp], ref_band[::nintp] - E_fermi, 'o', ms=2,
                               color=band_color, alpha=0.95, label="Ref")
            band_pre = ax.plot(self.eigenstatus["xlist"], self.eigenstatus["eigenvalues"] - E_fermi, color="tab:red",
                               lw=0.5, alpha=0.95, label="DeePTB")
        else:
            ax.plot(self.eigenstatus["xlist"], self.eigenstatus["eigenvalues"] - E_fermi, color="tab:red", lw=0.5,
                    alpha=0.95)
