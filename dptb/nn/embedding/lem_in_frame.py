...
from e3nn.o3 import Linear, SphericalHarmonics, FullyConnectedTensorProduct, TensorProduct, xyz_to_angles
...
from ..tensor_product import SO2_Linear, batch_wigner_D, _Jd, rotate_vector
...
class InitLayer(torch.nn.Module):
    ...
    def forward(self, edge_index, atom_type, bond_type, edge_sh, edge_length, edge_one_hot):
        ...
        if self.r_max_dict is None:
            ...
            for bond, ty in self.idp.bond_to_type.items():
                ...
                if mask.any():
                    ...
                    if self.cutoff_type == "cosine":
                        c_coeff = cosine_cutoff(edge_length[mask],
                                                0.5 * (self.r_max_dict[iatom] + self.r_max_dict[jatom]),
                                                r_start_cos_ratio=self.r_start_cos_ratio).flatten()
                    elif self.cutoff_type == "polynomial":
                        c_coeff = polynomial_cutoff(edge_length[mask],
                                                    0.5 * (self.r_max_dict[iatom] + self.r_max_dict[jatom]),
                                                    p=self.polynomial_cutoff_p).flatten()
                    else:
                        assert False, "Invalid cutoff type"
        ...
        latents = torch.zeros((edge_sh.shape[0], self.two_body_latent.out_features), dtype=edge_sh.dtype,
        ...
        latents = torch.index_copy(latents, 0, active_edges, cutoff_coeffs[active_edges].unsqueeze(-1) * new_latents)
...
class UpdateNodeInFrame(torch.nn.Module):
    ...
    根据 rotate_in 和 rotate_out 标志位手动处理 Node 特征的旋转和 Message 的回旋。
    """
    ...
    def __init__(
        ...
        self.sln_n = SeperableLayerNorm(irreps=self.node_irreps_in, eps=norm_eps, affine=True,
                                        normalization='component', std_balance_degrees=True, dtype=self.dtype,
                                        device=self.device)
        self.sln_e = SeperableLayerNorm(irreps=self.edge_irreps_in, eps=norm_eps, affine=True,
                                        normalization='component', std_balance_degrees=True, dtype=self.dtype,
                                        device=self.device)

        self._env_weighter = E3ElementLinear(irreps_in=self.irreps_out, dtype=dtype, device=device)
        ...
        if use_layer_onehot_tp:
            self.node_onehot_tp = FullyConnectedTensorProduct(irreps_in1=self.irreps_out,
    ...
    def forward(
        ...
        if wigner_D_all is None:
            ...
            if l_max > 0:
                ...
                wigner_D_all = batch_wigner_D(l_max, angle[0], angle[1], torch.zeros_like(angle[0]), _Jd)

        # 2. 准备 TP 输入
        center_node = norm_node_features[edge_center[active_edges]]
        neighbor_node = norm_node_features[edge_neighbor[active_edges]]

        # 如果 tp_rotate_in=False (中间层)，说明 Edge 已经是 Local，但 Node 还是 Global。
        # TP 不会进行旋转，所以我们必须手动将 Node 转到 Local 帧。
        if not self.tp_rotate_in:
            ...
            # Edge 特征保持不变（已经是 Local）
        # 如果 tp_rotate_in=True (第0层)，Node 和 Edge 都是 Global，TP 会统一旋转，无需手动操作。

        edge_input = torch.cat([center_node, norm_edge_features, neighbor_node], dim=-1)

        # 3. 执行 TP
        # 根据 init 里的 flags，TP 会决定是否旋转输入(如果 True) 和是否旋转输出(如果 True)
        ...
        # 此时 edge_messages 的状态：
        # 如果 tp_rotate_out=True (最后一层): 它是 Global
        # 如果 tp_rotate_out=False (其他层): 它是 Local

        # 4. 节点更新 (聚合)
        # 节点更新需要 Global 帧的消息进行聚合
        msg_for_node = edge_messages
        if not self.tp_rotate_out:
            # 如果输出保持在 Local，我们需要为了节点更新手动将其转回 Global
        ...
        # 5. 返回
        # 始终返回更新后的 Node (Global) 和 TP 输出的 Edge Message (根据层数可能是 Local 或 Global)
        # 供下一层作为 edge_features 使用
...
class LemInFrame(torch.nn.Module):
    def __init__(
        ...
        self.init_layer = InitLayer(
            idp=self.idp, num_types=self.n_atom, n_radial_basis=n_radial_basis, r_max=r_max, irreps_sh=irreps_sh,
            avg_num_neighbors=avg_num_neighbors, env_embed_multiplicity=env_embed_multiplicity,
            two_body_latent_channels=latent_channels, latent_dim=latent_dim, r_start_cos_ratio=r_start_cos_ratio,
            PolynomialCutoff_p=PolynomialCutoff_p, cutoff_type=cutoff_type, device=device, dtype=dtype,
            edge_one_hot_dim=edge_one_hot_dim, norm_eps=norm_eps, prune_edges_by_cutoff=prune_edges_by_cutoff,
        )
        ...
        for i in range(n_layers):
            if i == 0:
                ...
                # Layer 0: Global In -> Rotate In -> Local Out (No Rotate Out)
                rotate_in = True
                rotate_out = False
            else:
                irreps_in_layer = irreps_hidden
                # Layer > 0: Local In (No Rotate In)
                rotate_in = False

                if i == n_layers - 1:
                    # Last Layer: Local In -> Rotate Out -> Global Out
                    rotate_out = True
                else:
                    # Middle Layer: Local In -> No Rotate Out -> Local Out
                    rotate_out = False
        ...
        if self.use_out_onehot_tp:
            self.out_node_ele_tp = FullyConnectedTensorProduct(irreps_in1=self.node_irreps_out,
                                                               irreps_in2=f"{self.n_atom}x0e",
                                                               irreps_out=self.idp.orbpair_irreps)
            self.out_edge_ele_tp = FullyConnectedTensorProduct(irreps_in1=self.edge_irreps_out,
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        if self.prune_log_path is not None:
            with open(self.prune_log_path, "a") as f: f.write(f"{edge_index.shape[1]},{active_edges.shape[0]}\n")
        ...
        edge_feat_full = torch.zeros(edge_index.shape[1], self.idp.orbpair_irreps.dim, dtype=self.dtype,
                                     device=self.device)
