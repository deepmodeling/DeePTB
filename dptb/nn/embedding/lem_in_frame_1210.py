...
class UpdateNodeInFrame(torch.nn.Module):
    ...
    - onehot_mode:
        * "FullTP"    : 使用 FullyConnectedTensorProduct（当前默认）
        * "ElementTP" : 使用旧代码里那种按元素的 TensorProduct（不再写死 95）
    """
    ...
    def __init__(
        ...
        # ---- one-hot 调节的 TP（支持 FullTP / ElementTP）----
        if self.use_layer_onehot_tp:
            mode = self.onehot_mode
            if mode == "fulltp":
                # 当前实现：完整的 FullyConnectedTensorProduct
                ...
                # 参照旧代码：使用带 instructions 的 TensorProduct，但不再写死 95
                instructions = []
                ...
                raise ValueError(
                    f"Unknown onehot_mode={onehot_mode!r}, expected 'FullTP' or 'ElementTP'"
                )
...
class LemInFrame(torch.nn.Module):
    ...
    - onehot_mode:
        * "FullTP"    : use FullyConnectedTensorProduct 做 one-hot 调节（默认）
        * "ElementTP" : 参照旧代码的 TensorProduct（不再写死 95）
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f'onehot_mode: {onehot_mode}')
