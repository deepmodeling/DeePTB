...
class ComformerConv_edge(nn.Module):
    def __init__(self, in_channels: Union[int, Tuple[int, int]], out_channels: int,
        ...
        # Handle input channels tuple conversion
        if isinstance(in_channels, int):
            in_channels = (in_channels, in_channels)

        # Length embedding for edge features
        ...
        # Edge-specific key and value transformations for 3 neighbor positions
        ...
        # Output projection and message update networks
        self.lin_concate = nn.Linear(heads * out_channels, out_channels)
        ...
        # Key update network for attention computation
        ...
        print('Using invariant version of EPCNet (BatchNorm removed)')
    ...
    def forward(self, edge: Union[Tensor, PairTensor],
        ...
        # Handle tensor pair input
        if isinstance(edge, Tensor):
            edge = (edge, edge)

        # Compute query, key, value for central edge
        # Shape: [batch_size, 3, heads, out_channels]
        ...
        # Compute key and value for 3 neighboring edges
        key_y = torch.cat((
        ...
        # Process edge angle features
        edge_xy = self.lin_edge(edge_nei_angle).view(-1, 3, H, C)

        # Update keys with combined information and compute attention scores
        key = self.key_update(torch.cat((key_x, key_y, edge_xy), dim=-1))
        alpha = (query_x * key) / math.sqrt(self.out_channels)

        # Update message with combined value information
        out = self.lin_msg_update(torch.cat((value_x, value_y, edge_xy), dim=-1))

        # Apply attention weights (removed batch normalization)
        att = self.sigmoid(alpha)
        out = out * att

        # Aggregate across heads and neighbors
        out = out.view(-1, 3, self.heads * self.out_channels)
        out = self.lin_concate(out)
        out = out.sum(dim=1)

        # Return final output with residual connection (removed batch normalization)
