...
# from comforemr.models.utils import softmax, RBFExpansion
...
class ComformerConv_edge(nn.Module):
    def __init__(self, in_channels: Union[int, Tuple[int, int]], out_channels: int,
        ...
        # 可选：提供开关，控制是否在前向中强制 BN 使用 batch 统计
        self.force_bn_batch_stats = True
        self.freeze_bn_running_stats = True  # True 时前向不更新 running_mean/var

    # --- 新增：仅切换本模块内 BN 的训练/统计策略 ---
    def _enter_bn_train_mode(self):
        """只把本模块内的 BatchNorm1d 切到训练态，必要时冻结 running stats。"""
        self._bn_prev = []  # 保存 (module, prev_training, prev_momentum)
        for m in self.modules():
            if isinstance(m, nn.BatchNorm1d):
                self._bn_prev.append((m, m.training, m.momentum))
                m.train(True)  # 用 batch 统计
                if self.freeze_bn_running_stats:
                    # 让 running_mean/var 不被本次前向刷新
                    m.momentum = 0.0

    def _leave_bn_train_mode(self):
        """恢复 BN 的训练标志与 momentum。"""
        if hasattr(self, "_bn_prev") and self._bn_prev is not None:
            for m, prev_training, prev_momentum in self._bn_prev:
                m.train(prev_training)
                m.momentum = prev_momentum
            self._bn_prev = None

    def forward(self, edge: Union[Tensor, PairTensor],
                edge_nei_len: Optional[Tensor] = None,
                edge_nei_angle: Optional[Tensor] = None):

        # 仅在本层内强制 BN 用 batch 统计，不影响其他模块的 train/eval
        if self.force_bn_batch_stats:
            self._enter_bn_train_mode()
        try:
            H, C = self.heads, self.out_channels
            ...
            # 这里的 BN 现在会用 batch 统计，但不会改动 running_mean/var（若冻结）
            att = self.bn_att(alpha.view(-1, self.out_channels)).view(-1, 3, self.heads, self.out_channels)
            out = out * self.sigmoid(att)

            out = out.view(-1, 3, self.heads * self.out_channels)
            out = self.lin_concate(out)
            out = out.sum(dim=1)

            out = self.bn(out)  # 同理，这里 BN 也用 batch 统计
            return self.softplus(edge[1] + out)

        finally:
            if self.force_bn_batch_stats:
                self._leave_bn_train_mode()
