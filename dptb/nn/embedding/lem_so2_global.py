...
from e3nn.o3 import Linear, SphericalHarmonics, FullyConnectedTensorProduct, TensorProduct, ElementwiseTensorProduct
...
from dptb.nn.tensor_product import SO2_Linear, SO2_Attention
...
# ==========================================
# Helper Classes with NaN Protection
# ==========================================
...
class BaseMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.residual:
            out = x + out
        # Protection against exploding activations
...
class NodeColor(nn.Module):
    ...
    def forward(self, node_feat, node_pos, batch, edge_index=None):
        # Protection: handle empty batch or single atom case gracefully
        if node_pos.shape[0] == 0:
            return torch.zeros_like(node_feat)

        center = scatter_mean(node_pos, batch, dim=0)
        # Protection: if scatter returns NaNs (unlikely but possible with bad indices)
...
class InitLayerGlobal(torch.nn.Module):
    ...
    def forward(self, edge_index, atom_type, bond_type, edge_sh, edge_length, edge_one_hot, msg_cpl):
        ...
        node_features = scatter(
            ...
            dim=0,
            dim_size=atom_type.shape[0]  # Explicit dim size
        )
...
class UpdateNodeGlobal(torch.nn.Module):
    def __init__(
        ...
        self.sln = SeperableLayerNorm(irreps=self.irreps_in, eps=norm_eps, affine=True, normalization='component',
    ...
    def forward(self, latents, node_features, edge_features, atom_type, node_onehot, edge_index, edge_vector,
        ...
        # Protection for empty edges
        if active_edges.numel() == 0:
            # Just return identity update
            return node_features, msg_cpl

        new_node_features = self.sln(node_features)

        # Ensure vectors are not NaN before TP
        ...
        # Soft clamp coeffs to avoid extreme scaling causing NaNs down the line
        # exp(tanh) -> range [0.36, 2.7], usually safe for scaling
        # or just simple tanh + 1.0 to keep it around 1.0
        # coeffs = torch.tanh(coeffs) + 1.0
...
class LayerGlobal(torch.nn.Module):
    def __init__(
        ...
        self.edge_update = UpdateEdge(
            ...
            so2_attn_aggressive=so2_attn_aggressive,
            norm_eps=norm_eps
        )
...
@Embedding.register("lem_so2_global")
class LemSO2Global(torch.nn.Module):
    def __init__(
            self,
        ...
        super(LemSO2Global, self).__init__()
...
class UpdateEdge(torch.nn.Module):
    def __init__(
        ...
        use_layer_onehot_tp: bool = True,
        use_interpolation_tp: bool = False,
        so2_attn_aggressive: bool = False,
        edge_one_hot_dim: int = 128,
        res_update_ratios: Optional[List[float]] = None,
        res_update_ratios_learnable: bool = False,
        dtype: Union[str, torch.dtype] = torch.float32,
        ...
        self.latents_att = SO2_Attention(
            node_irreps=self.node_irreps_in,
            latent_dim=latent_dim
        )
        self.ln_latent = torch.nn.LayerNorm(latent_dim)
        self.ln_latent_score = torch.nn.LayerNorm(latent_dim)
        self.so2_attn_aggressive = so2_attn_aggressive
        if self.so2_attn_aggressive:
            self.latents_mlp_1 = ScalarMLPFunction(
                mlp_input_dimension=latent_dim + latent_dim,
                mlp_output_dimension=latent_dim,
            ...
            self.latents_mlp_1 = ScalarMLPFunction(
                mlp_input_dimension=latent_dim + latent_dim + self.irreps_out[0].dim,
                mlp_output_dimension=latent_dim,
                mlp_latent_dimensions=latent_channels,
                mlp_nonlinearity="silu",
                mlp_initialization="uniform",
    ...
    def forward(self, latents, node_features, node_onehot, edge_features, edge_index, edge_vector, cutoff_coeffs, active_edges, edge_one_hot, wigner_D_all):
        ...
        # update latent
        latent_att_score = self.latents_att(
            node_features=new_node_features,
        ...
        normed_latent = self.ln_latent(latents[active_edges])
        normed_latent_score = self.ln_latent_score(latent_att_score)
        if self.so2_attn_aggressive:
        ...
        # print('===================================')
        # print('===================================')
        # print(cutoff_coeffs[active_edges])
        # print('===================================')
        # print('===================================')
        #
        # raise RuntimeError
